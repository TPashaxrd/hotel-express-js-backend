...
const deleteHotel = async(req, res, next) => {
    ...
    try {
        ...
        res.status(200).json({message: "Successyfull Deleted..."})
    } catch (error) {
        res.status(500).json({message: error})
    }
}

const getSignleHotel = async(req, res, next) => {
    const { id } = req.params
    try {
...
module.exports = {typeByCity, typeByCount, createHotel, updateHotel, deleteHotel, getSignleHotel, getAllHotel}